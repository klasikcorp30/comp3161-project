...
fs.appendFile(
    ...
    ${laptops.map((item) => {
            ...
            return `INSERT INTO laptop_details(prod_brand,prod_model,prod_processor,RAM,storage,OS,price,branch) VALUES('${item.Brand}', '${item.Model}', '${item.Processor}', '${item.Ram}', '${item.Storage}', '${os}', ${price},'${argv[2]}');\n`
        })}`,err => console.log(err));
